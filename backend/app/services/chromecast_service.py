...
class ChromecastService:
    ...
    def get_devices(self) -> list[ChromecastDevice]:
        ...
        for device_id, cast in self._devices.items():
            try:
                ...
                cast_info = cast.cast_info if hasattr(cast, "cast_info") else None
                ip_address = str(cast_info.host) if cast_info else "unknown"
                port = cast_info.port if cast_info else 8009
                devices.append(
                    ChromecastDevice(
                        id=device_id,
                        name=cast.name,
    ...
    async def play_url(
        self, device_id: str, url: str, content_type: str = "audio/mp3"
    ) -> bool:
    ...
    async def quit_app(self, device_id: str) -> bool:
        """Quit the current app/stop casting on a device."""
        cast = self._devices.get(device_id)
        if not cast:
            return False

        loop = asyncio.get_event_loop()
        await loop.run_in_executor(None, cast.quit_app)
        return True

    def is_casting(self, device_id: str) -> bool:
        """Check if a device is currently casting (not idle)."""
        cast = self._devices.get(device_id)
        if not cast:
            return False
        return not cast.is_idle
