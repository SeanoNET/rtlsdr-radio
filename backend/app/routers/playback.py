...
from app.models import PlaybackStartRequest, PlaybackStatus, Modulation, SpeakerType
...
async def start_playback(playback_req: PlaybackStartRequest, request: Request):
    """Start playback to a Chromecast or LMS device."""
    service = request.app.state.playback_service
    
    # Parse unified speaker ID if it contains a prefix
    if ":" in playback_req.device_id:
        device_type, device_id = parse_speaker_id(playback_req.device_id)
    else:
        device_type = playback_req.device_type
        device_id = playback_req.device_id
    
    # Determine frequency to tune to
    ...
    if frequency is None:
        raise HTTPException(
            status_code=400,
            detail="Either station_id or frequency must be provided",
        )
    ...
    if not success:
        raise HTTPException(
            status_code=500,
            detail="Failed to start playback. Check device and RTL-SDR connection.",
        )
...
@router.post("/tune")
async def change_frequency(tune_req: PlaybackStartRequest, request: Request):
    ...
    if frequency is None:
        raise HTTPException(status_code=400, detail="Frequency or station_id required")
    
    success = await service.change_frequency(frequency, modulation)
    if not success:
        raise HTTPException(status_code=400, detail="Cannot change frequency - not playing")
    
    return {"message": f"Tuned to {frequency} MHz", "frequency": frequency}
