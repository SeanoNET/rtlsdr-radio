...
from app.models import Speaker, SpeakerType, VolumeRequest, MuteRequest
...
async def set_power(speaker_id: str, request: Request, power: bool = True):
    """Power on/off a speaker (LMS only)."""
    speaker_type, device_id = parse_speaker_id(speaker_id)
    ...
    if speaker_type == SpeakerType.LMS:
        ...
        raise HTTPException(
            status_code=400, 
            detail="Power control only available for LMS players"
        )
